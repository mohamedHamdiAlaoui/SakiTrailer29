...
export function ensureProductSeed() {
  ...
  if (storedProducts.length === 0 || seedVersion < PRODUCT_SEED_VERSION) {
    ...
    const customProducts = storedProducts
      ...
      .map((product) => ({ ...product, category: normalizeCategory((product as { category?: unknown }).category) }));
  ...
  return storedProducts.map((product) => ({ ...product, category: normalizeCategory((product as { category?: unknown }).category) }));
}
